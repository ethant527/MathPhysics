...
class ODESolve:
    ...
    # crank nicolson (trapezoidal rule) - second order implicit method, unconditionally stable for linear problems
    @staticmethod
    def crank_nicolson(f: Callable, y0: np.ndarray, t: np.ndarray, iters: int = 5) -> np.ndarray:
        ...
        for i in range(len(t) - 1):
            ...
            for _ in range(iters):
                ...
                # solving the implicit equation via fixed-point iteration
                f_current = np.asarray(f(t[i], y[i]))
                f_next    = np.asarray(f(t[i+1], y_next))
                y_next    = y[i] + (h / 2) * (f_current + f_next)
...
if __name__ == "__main__":
   ...
   # example derivative for a simple harmonic oscillator
    ...
    results_custom = ODESolve.heun_euler(harmonic_oscillator, y_initial, t_steps)
